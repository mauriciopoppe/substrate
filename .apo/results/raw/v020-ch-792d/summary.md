...
- **Subsystem Health Triage**:
  - In Subsystem B (`cmd/ateom-microvm/internal/ch`), `BenchmarkCopySparseRegions` handles bulk differential memory processing with low latency but continues to trigger a heap allocation per operation (`1 allocs/op`).
...
| :--- | :--- | :--- | :--- |
| `FEATURE_CH_SPARSE_STAT_BYPASS` | `true` | `cmd/ateom-microvm/internal/ch/merge.go` | `apo-provider-go-compiler` |
