...
- **Current Pareto Frontier**:
  - `v003-ategcs-zstd-chunk-pool-a8d7`: `CPU latencies (ch, ategcs, tarutil)` = 49,489,457 ns/op (-16.8% vs baseline), `total_bytes_per_op` = 28,807,617 B/op (-74.3% vs baseline), `total_allocs_per_op` = 14,085 allocs/op, `benchmark_failures` = 0 (Outcome: KEEP / Champion Baseline).
...
- **Subsystem Health Triage**:
  ...
  - In `copySparseRegions` (`cmd/ateom-microvm/internal/ch/merge.go`), every call allocates a fresh 1 MiB buffer via `buf := make([]byte, 1<<20)`.
...
- **Refuted Patterns Avoided**:
  - Avoided modifying files outside authorized scope (`cmd/ateom-microvm/internal/ch/merge.go` is strictly within authorized scope).
...
- **Option B (Explore Path - Archetype Action)**: `[ACTION: SUBSYSTEM_PIVOT]` and `[ACTION: CODE_REFACTOR]` targeting `cmd/ateom-microvm/internal/ch/merge.go`. Replaces per-call 1 MiB buffer allocation in `copySparseRegions` with a package-level `sync.Pool` (`sparseCopyBufPool`).
...
- **Expected Gain & Technical Rationale**:
  ...
  - Reduces total heap bytes across the benchmark suite by ~2.1 MiB/op and relieves GC mark worker duty cycle.
