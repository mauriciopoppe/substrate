...
- **Current Pareto Frontier**:
  - `v003-ategcs-zstd-chunk-pool-a8d7` (Parent Champion): `composite_ns_per_op` = 49,489,457 ns/op, `composite_bytes_per_op` = 28,807,617 B/op, `composite_allocs_per_op` = 14,085 allocs/op, `benchmark_failures` = 0.
...
- **Option B (Explore Path - Archetype Action)**: `[ACTION: SUBSYSTEM_PIVOT]` and `[ACTION: CODE_REFACTOR]` targeting `substrate/internal/tarutil/tarutil.go`. Implements a `sync.Pool` based `xattrBufPool` buffer to recycle byte slices when extracting overlayfs attributes `buf` and `val` in `readOverlayXattrs`.
...
- **Expected Gain & Technical Rationale**:
  ...
  - Reduces `composite_bytes_per_op` and `composite_allocs_per_op` roughly proportionally to the total number of files in the simulated tar archive workload.
...
- **Outcome**: VALIDATED
- **Strategy**: EXPLORE
- **Rationale**: Validated pure Go code refactor in `substrate/internal/tarutil/tarutil.go`. Reuses 8KB extended attribute buffers via package-level `sync.Pool` (`xattrBufPool`) with proper local scope `defer Put`, dynamic capacity expansion, and immutable string copy extraction, eliminating continuous heap allocations during rootfs archive xattr scanning while preserving concurrency safety and data integrity.

### 2. Safety Rubric & Checklist Grading
- **Deduplication Check**: PASS (Unique code mutation targeting tarutil xattr buffer allocation)
- **Physical Diff Audit**: PASS (Surgically scoped strictly to `substrate/internal/tarutil/tarutil.go` within authorized scope in `prompts/objective.md`; no diff leakage)
...
| :--- | :--- | :--- | :--- |
| `tarutil.go (xattrBufPool sync.Pool)` | `CODE_REFACTOR` | `substrate/internal/tarutil/tarutil.go` | `apo-provider-go-compiler` |
