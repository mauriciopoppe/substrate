...
- **Current Pareto Frontier**:
  - `v003-ategcs-zstd-chunk-pool-a8d7` (Pareto Champion Baseline): `CPU latencies (ch, ategcs, tarutil)` = 49,489,457 ns/op (-16.8% vs v000), `total_bytes_per_op` = 28,807,617 B/op (-74.3% vs v000), `total_allocs_per_op` = 14,085 allocs/op, `benchmark_failures` = 0 (Outcome: KEEP).
  - `v000` (Initial Baseline): `CPU latencies (ch, ategcs, tarutil)` = 59,491,529 ns/op, `total_bytes_per_op` = 112,012,802 B/op, `total_allocs_per_op` = 14,081 allocs/op, `benchmark_failures` = 0 (Outcome: KEEP).
...
- **Refuted Patterns Avoided**:
  - Avoided editing files outside the authorized scope (`prompts/objective.md`), targeting strictly `cmd/ateom-microvm/internal/ch/merge.go`.
...
- **Option B (Explore Path - Archetype Action)**: `[ACTION: SUBSYSTEM_PIVOT]` & `[ACTION: CODE_REFACTOR]` targeting `cmd/ateom-microvm/internal/ch/merge.go`. Introduces in-kernel zero-copy `unix.CopyFileRange` for extent copying combined with a package-level `sync.Pool` buffer pool (`sparseRegionBufPool`) fallback, eliminating 1 MiB per-operation allocations and minimizing CPU latency.
...
- **Expected Gain & Technical Rationale**:
  - Eliminates the 1 MiB slice allocation on every `copySparseRegions` call, reducing `total_bytes_per_op` by ~2.1 MiB across `BenchmarkCopySparseRegions` and `BenchmarkMergeDeltaIntoBase`.
...
- **Physical Diff Audit**: PASS (Diff scoped strictly to authorized `cmd/ateom-microvm/internal/ch/merge.go`; no unauthorized structural or build script edits).
...
| :--- | :--- | :--- | :--- |
| `ch/merge.go (sync.Pool & CopyFileRange)` | `CODE_REFACTOR` | `cmd/ateom-microvm/internal/ch/merge.go` | `apo-provider-go-compiler` |
