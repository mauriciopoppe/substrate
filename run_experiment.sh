#!/bin/bash
# ==============================================================================
# WorkQueue Benchmark Execution Script for Substrate Micro-VM Memory & Checkpoint
...
# 4. Deploys custom ateom-microvm and atelet images if built by build.sh
...
echo "Starting Substrate Study 1 Micro-VM Benchmark execution in ${WORKSPACE_DIR} (${BENCHMARK_ITERATIONS} iterations)..."
...
echo "Reconciling benchmark worker pool and glutton ActorTemplate..."
WORKLOAD_TEMPLATES="glutton" ./substrate/benchmarking/workloads/deploy.sh \
  --deploy \
  --worker-count="${WORKER_COUNT:-1}" \
  --sandbox-class="${SANDBOX_CLASS:-microvm}" \
  --actor-memory="${ACTOR_MEMORY:-1536Mi}"

if [ -n "${ATEOM_MICROVM_IMAGE:-}" ]; then
...
TEST_NAME="${TEST_NAME:-glutton_mem_1gi_microvm}"
...
for iter in $(seq 1 "${BENCHMARK_ITERATIONS}"); do
  ...
  # Clean up any leftover runner jobs or pods from previous iteration
  kubectl delete jobs,pods -n benchmarking --all --wait=false 2>/dev/null || true
  ...
  # Reset benchmark worker pool to clear internal sandbox cache
  echo "Resetting benchmark worker pool for iteration ${iter}..."
  kubectl rollout restart deployment/benchmark-ateom -n benchmark-workloads >/dev/null 2>&1 || true
  kubectl rollout status deployment/benchmark-ateom -n benchmark-workloads --timeout=120s >/dev/null 2>&1 || true

  # Flush dirty writeback buffers and drop kernel page caches on the benchmark node
  ...
  template:
    ...
    spec:
      ...
      - name: runner
        ...
        - "${TRACE_PROBABILITY}"
        - "--mem-target"
        - "${BENCHMARK_MEM_TARGET:-1Gi}"
        - "--mem-churn"
        - "${BENCHMARK_MEM_CHURN:-64Mi}"
        - "--mem-read"
        - "${BENCHMARK_MEM_READ:-all}"
        env:
...
try:
    ...
    metrics = {
        "resume_actor_p95_ms": 0.0,
        "read_ram_after_resume_p95_ms": 0.0,
    ...
    if stats_files and os.path.exists(stats_files[0]):
        with open(stats_files[0], "r") as f:
            ...
            for row in reader:
                ...
                if name == "ResumeActor":
                    metrics["resume_actor_p95_ms"] = p95_val
                elif name == "GluttonReadRAM":
                    metrics["read_ram_after_resume_p95_ms"] = p95_val
    ...
    print(f"Successfully generated {iter_dir}/summary.json with Study 1 metrics:", metrics)
    ...
    if os.path.exists(iter_file):
        try:
            with open(iter_file, "r") as f:
                data = json.load(f)
                if data.get("metrics", {}).get("resume_actor_p95_ms", 0.0) > 0:
...
# Sort iterations by primary objective (resume_actor_p95_ms ascending)
iter_summaries.sort(key=lambda item: item[1].get("metrics", {}).get("resume_actor_p95_ms", float("inf")))
...
# Select the entire median run (preserving causal consistency across metrics)
top_summary = {
    "status": "COMPLETED",
