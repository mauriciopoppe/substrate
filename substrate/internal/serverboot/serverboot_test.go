...
func TestPprofAbsentUnlessEnabled(t *testing.T) {
	mux := metricsMux(MetricsServerOptions{})
	for _, path := range []string{"/debug/pprof/", "/debug/pprof/cmdline", "/debug/pprof/profile", "/debug/pprof/symbol", "/debug/pprof/trace"} {
		if got := getCode(t, mux, path); got != http.StatusNotFound {
			t.Errorf("%s without EnablePprof = %d, want %d", path, got, http.StatusNotFound)
		}
	}
}

func TestPprofPresentWhenEnabled(t *testing.T) {
	mux := metricsMux(MetricsServerOptions{EnablePprof: true, ProfileFraction: 10, BlockProfileRate: 100})
	// /debug/pprof/ serves a 200 index page
	if got := getCode(t, mux, "/debug/pprof/"); got != http.StatusOK {
		t.Errorf("/debug/pprof/ with EnablePprof = %d, want %d", got, http.StatusOK)
	}
	// /debug/pprof/cmdline serves 200
	if got := getCode(t, mux, "/debug/pprof/cmdline"); got != http.StatusOK {
		t.Errorf("/debug/pprof/cmdline with EnablePprof = %d, want %d", got, http.StatusOK)
	}
}
