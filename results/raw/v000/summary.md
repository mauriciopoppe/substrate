...
# Baseline Calibration Summary: v000

## 1. System Architecture & Ingress Routing Flow
...
| **Trial Identifier** | Baseline (`v000`) | Calibration run |
...
During the active testing stage, a 30-second pprof CPU profile was captured from `atenet-router` on port `:19090` (`results/raw/v000/profiles/cpu.pb.gz`):
...
| Trial ID | Hyperparameter / Mutation Summary | Target Throughput | Latency P95 (ms) | Latency P99 (ms) | SLA Status | Outcome / Delta vs Baseline |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `v000` | Baseline Calibration Reference | 5248.0 RPS | 18.96 ms | 49.95 ms | PASS | **KEEP (Champion Baseline)** |
